...
def add_linkLayer(apdu_list, CA_text='00', SA_text='00000001', logic_addr=0, SA_type=0, C_text='43'):
    ...
    hcs_calc = get_fcs(text2list(hcs_clac_aera_text))
    hcs_calc = ((hcs_calc << 8) | (hcs_calc >> 8)) & 0xffff  # 低位在前
    fcs_calc_aera_text = hcs_clac_aera_text + '{0:04X}'.format(hcs_calc) + list2text(apdu_list)
    fcs_calc = get_fcs(text2list(fcs_calc_aera_text))
    fcs_calc = ((fcs_calc << 8) | (fcs_calc >> 8)) & 0xffff  # 低位在前

    return ('68' + fcs_calc_aera_text + '{0:04X}'.format(fcs_calc) + '16').replace(' ', '')
